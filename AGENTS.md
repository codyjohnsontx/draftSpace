...
- The floating style bar has a width budget: no control it draws may leave the bar or the window, from 1440px down to 375px, measured by `tests/e2e/style-inspector.spec.ts`. It wraps rather than scrolls, so a control added to it costs a row rather than silently disappearing off the right-hand end - check that test rather than the bar by eye, and note that the bar's own box proves nothing, since `width: max-content` under a `max-width` stays on screen however far its contents run past it. Only the floating shape is compact: the sidebar shows every colour inline.
