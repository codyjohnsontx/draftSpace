...
- Styling the app shell: `src/app/globals.css` opens with the token layer - colour ramps, radii, weights, type scale, spacing, elevation and z-order - and nothing below that block carries a hex literal, a raw radius, a raw weight or a raw padding. Add a token rather than a literal; `sed -n '/^}/,$p' src/app/globals.css | grep -oE '#[0-9a-fA-F]{3,8}'` should stay empty. The landing page keeps its `.landing-*` scoping but draws on the same tokens.
- The floating style bar has a width budget: it must fit at 1440px with nothing hidden, measured by `tests/e2e/style-inspector.spec.ts`. It wraps rather than scrolls, so a control added to it costs a second row rather than silently disappearing off the right-hand end - check that test rather than the bar by eye. Only the floating shape is compact: the sidebar shows every colour inline.
