...
**One tab owns a board.** Where the Web Locks API is available, only the tab holding the board's claim may edit or save it and every other tab on the same board is read-only; without Web Locks the claim cannot be enforced, so each tab claims outright rather than being stranded read-only with no way back. One tab that holds no claim is still editable by design: a tab that cannot write to storage at all keeps editing its in-memory draft, precisely because a tab that cannot write cannot overwrite anyone. Otherwise edit rights are never wider than the claim actually held at that instant, mid-transition included, which is why access narrows to `pending` before a lease is released rather than after the next one lands, and why work resuming after an `await` proves it still holds the same lease (`boardClaimIsCurrent`) rather than checking the board id, which a switch away and back reuses. Holding the claim makes a write exclusive, not current, so a write path that cannot reload first must check that the stored record has not moved on. Two things follow. Any new control that mutates the document must gate on `useCanEditBoard()` / `canEditBoard()` (`src/hooks/use-can-edit-board.ts`) rather than reading the collaboration store directly, and `dispatchCommand` enforces the same rule for local commands as a backstop. Any new write path must run only when the tab owns the board, and anything that changes which board is open must hand the claim over with it. See "One tab owns a board" in `docs/architecture.md` for the design and the alternatives that were rejected.
