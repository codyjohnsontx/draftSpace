...
export function BoardSwitcher({ controller }: { controller: PersistenceController }) {
  ...
  const choose = (board: BoardSummary) => {
    setOpen(false); buttonRef.current?.focus();
    if (board.id !== openBoardId) void controller.openBoard(board.id);
  };
  ...
  return <div className="board-switcher" ref={wrapRef} onKeyDown={(event) => {
    ...
    <button ref={buttonRef} type="button" className="board-switcher-trigger" aria-label="Open a board" aria-haspopup="menu" aria-expanded={open} aria-controls="board-switcher-menu" onClick={() => setOpen((value) => !value)}>
    ...
    {open && <div className="board-switcher-popover" id="board-switcher-menu" role="menu" aria-label="Boards in this browser">
      ...
      {boards.map((board) => {
        ...
        return <button key={board.id} type="button" role="menuitemradio" aria-checked={board.id === openBoardId} className="board-switcher-option" onClick={() => choose(board)}>
          <span><strong>{board.name.trim() || "Untitled board"}</strong><small>{elementSummary(board.elementCount)}{updated ? ` · ${updated}` : ""}</small></span>
