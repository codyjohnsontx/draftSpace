...
export function CanvasWorkspace() {
  ...
  useEffect(() => {
    ...
    const down = async (event: KeyboardEvent) => {
      ...
      else if (mod && key === "v" && guestCanEdit) { event.preventDefault(); const pasted = useBoardStore.getState().pasteElements(await readElements()); if (pasted.length) useSessionStore.getState().setSelected(pasted); }
