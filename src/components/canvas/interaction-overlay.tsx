...
export function InteractionOverlay({ bounds, marquee, viewport }: { bounds: Bounds | null; marquee: Bounds | null; viewport: Viewport }) {
  ...
  return <svg className="interaction-overlay" aria-hidden="true">
    ...
    {bounds && (() => { const b = rect(bounds); return <g><rect className="selection-frame" {...b} />{handles.map((h) => <rect key={h.id} data-resize-handle={h.id} className="resize-handle" x={b.x + b.width * h.x - 5} y={b.y + b.height * h.y - 5} width="10" height="10" style={{ cursor: h.cursor }} />)}</g>; })()}
  </svg>;
}
