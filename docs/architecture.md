...
A browser can hold more than one board, so the repository's summary listing is surfaced beside the board name and the same last-opened key is what switching writes: the board picked is the board the next load opens. Switching flushes and drains the outgoing board's autosave before board state changes, because anything still inside the debounce belongs to that document and nothing afterwards would ever write it; history and selection are dropped with it, and no stored record is rewritten. Only records that parsed are listed, and one that has become unreadable between listing and opening leaves the open board exactly where it was rather than escalating a board the user did not ask about to the recovery screen. With a single stored board the control is not rendered at all, so an ordinary session gains no menu that can only ever name the board already on screen.
